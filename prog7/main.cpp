...
void arrayToOrderedList(OrderedList<Student> *l, Student array[], int first, int last);
...
int main(int argc, char** argv)
{
	...
	Student *tempArray = (Student*) malloc(numInRoster * sizeof(Student));
	
	for(int x = 0; x < numInRoster; x++)
	{ /* go through each space craft in roster */
		...
		tempArray[x] = Student(tempLn, tempFn, tempQz, tempHw, tempFx, qzWeight, hwWeight, fxWeight);
		
	} /* go through each student in roster */
	
	arrayToOrderedList(l, tempArray, 0, numInRoster - 1);
...
void printStudent(Student s)
{
	cout << s.toString() << " - " << s.letterGrade() << endl;
} /* print a student */
...
void arrayToOrderedList(OrderedList<Student> *l, Student array[], int first, int last)
{
	...
	/* if array is 2 or fewer, insert in a loop */
	if(last - first < 2)
	{
		for(int i = first; i <= last; i++)
		{
			Student *s = new Student(array[i]);
			l->insert(s);
		}
		return;
	}
	...
	Student *s = new Student(array[mid]);
	l->insert(s);
	
	/* recursively call left side */
	arrayToOrderedList(l, array, first, mid - 1);
	
	/* recursively call right side */
	arrayToOrderedList(l, array, mid + 1, last);
	
	return;
} /* array to ordered list */
